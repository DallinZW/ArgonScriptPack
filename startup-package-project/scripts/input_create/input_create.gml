function input_create() {
	...
	global.INPUT_KEYBOARD_KEYS[the_input_action]	= the_input_hotkey;
	global.INPUT_GAMEPAD_KEYS[the_input_action]	= the_input_gamepad_key;
	global.INPUT_MOUSE_KEYS[the_input_action]		= the_input_mouse_key;

	// Then, we'll set this input action's state to "none" for all players.
	for ( var p = 0; p < global.MAX_PLAYERS; p++) {
		global.INPUT_STATES[p][the_input_action]	= input_state.none;
	}
